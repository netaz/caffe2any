...
def test_alexnet_csv():
    ...
    assert ret==0
    ret = filecmp.cmp("output/alexnet.deploy.prototxt.csv", "tests/golden/alexnet.deploy.prototxt.csv")
    assert ret==True

def test_googlenet_v1_csv():
    ret = os.system("python caffe2any.py examples/googlenetv1.deploy.prototxt -p csv -d bfs")
    assert ret==0
    ret = filecmp.cmp("output/googlenetv1.deploy.prototxt.csv", "tests/golden/googlenetv1.deploy.prototxt.csv")
    assert ret==True

def test_resnet50_csv():
    ret = os.system("python caffe2any.py examples/ResNet-50-deploy.prototxt -p csv -d bfs")
    assert ret==0
    ret = filecmp.cmp("output/ResNet-50-deploy.prototxt.csv", "tests/golden/ResNet-50-deploy.prototxt.csv")
    assert ret==True
